...
emissions_long_subset = emissions_long_1990_2022.query(
    "country.isin(['Algeria','Egypt','Ethiopia','Nigeria','South Africa'])"
)

# emissions_long_subset.sort_values("country")
...
fig_chart = px.line(
    ...
    y="emissions",
    color="country",
    title="Annual CO2 Emissions (1990-2022): A Focus on Egypt, Ghana, Kenya, Nigeria and South Africa",
    labels={
        "year": "YEAR",
        "emissions": "Carbon Dioxide Total Emission (In Mil Tonnes per capita)",
    },
    markers=True,
)

for country in emissions_long_subset["country"].unique():
    subset = emissions_long_subset[emissions_long_subset["country"] == country]
    last_point = subset.iloc[-1]  # Grab the last point for the country
    fig_chart.add_annotation(
        x=last_point["year"],
        y=last_point["emissions"],
        text=country,
        showarrow=False,
        xshift=30,
    )
...
# emissions_long_1990_2022.describe()
...
```{python}
# | echo: false
print("GLOBAL CARBON DIOXIDE EMISSION (1990 - 2022)")
show(table_df)
```
